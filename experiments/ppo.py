...
if action_type == 'discrete':
    ...
    agent = MLP(input_dim=env.state_dim, output_dim=output_dim, n_layers=n_layers, hidden_dim=hidden_dim, max_action=max_action)
else:
    from agents.mlp_continuous import MLPC
    agent = MLPC(input_dim=env.state_dim, output_dim=output_dim, n_layers=n_layers, hidden_dim=hidden_dim, max_action=max_action)
...
rollout_buffer = RolloutBuffer(state_dim=env.state_dim, action_dim=output_dim, size=rollout_buffer_size, gamma=gamma, lambda_=lambda_, batch_size=config['batch_size'])
